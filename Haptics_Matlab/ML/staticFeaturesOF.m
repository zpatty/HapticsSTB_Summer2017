...
%%%%% This version also allows certain features to be removed

function feat = staticFeaturesOF(data,rounding,omit)
%%%%% omit = 1 removes acceleration features
%%%%% omit = 2 removes force features
...
% 		fAcc2Prod = acc3.*fMag;
       ...
       %%
        ...
        if omit == 1 % omits PCA features (not affected here)
        ...
        if omit == 2 % omits force features
            raw_featuresABS = {abs(acc1),abs(acc1H),abs(acc1L),abs(acc2),abs(acc2H),abs(acc2L),abs(acc3),abs(acc3H),abs(acc3L),... %acceleration data (total, high, low)
                abs(accHProd),abs(accProd),abs(accLProd),... %product features
                r1, p1, r2, p2, r3, p3, ... %orientation data (roll and pitch)
                diff(r1), diff(p1), diff(r2), diff(p2), diff(r3), diff(p3)}; %angular velocity data (roll rate and pitch rate)
            
            raw_features = {acc1,acc1H,acc1L,acc2,acc2H,acc2L,acc3,acc3H,acc3L,... %acceleration data (total, high, low)
                accHProd,accProd,accLProd,... %product features
                r1, p1, r2, p2, r3, p3, ... %orientation data (roll and pitch)
                diff(r1), diff(p1), diff(r2), diff(p2), diff(r3), diff(p3)};
            
            feat(k).int = [extract_feature(@(x) trapz(0:1/3000:(length(x)-1)/3000,x), raw_featuresABS{1:12});... 
                       extract_feature(@(x) trapz(0:1/100:(length(x)-1)/100,x), raw_featuresABS{13:24})];
        end
        
        if omit == 3 % omits PCA and force features
            raw_featuresABS = {abs(acc1),abs(acc1H),abs(acc1L),abs(acc2),abs(acc2H),abs(acc2L),abs(acc3),abs(acc3H),abs(acc3L),... %acceleration data (total, high, low)
                abs(accHProd),abs(accProd),abs(accLProd),... %product features
                r1, p1, r2, p2, r3, p3, ... %orientation data (roll and pitch)
                diff(r1), diff(p1), diff(r2), diff(p2), diff(r3), diff(p3)}; %angular velocity data (roll rate and pitch rate)
