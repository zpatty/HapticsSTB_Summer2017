...
%%%%% This version omits features

function [b,C,results,selectFeat,classifScore] = randomForestTrainRoundOF(rounding,TrainTest,metric,leaf,nTrees,time,folder,omit)
%     rounding = 1;
%     xVal = 1;
%     metric = 1;
%     leaf = 10;
%     nTrees = 100;
    clearvars -except design TrainTest rounding metric leaf nTrees time folder omit; close all;
    ...
    if TrainTest == 0 % create CV partition file for train/test split
        if rounding == 0 % use median value for GEARS scores
            if omit == 1 % PCA features removed
                file = strcat('randForest',num2str(nTrees),'TreesMedCVPartNoPCA',time,'.mat');
            elseif omit == 2 % force features removed
                file = strcat('randForest',num2str(nTrees),'TreesMedCVPartNoForce',time,'.mat');
            elseif omit == 3 % PCA and force features removed
                file = strcat('randForest',num2str(nTrees),'TreesMedCVPartNoForcePCA',time,'.mat');
            end
        elseif rounding == 1 % use mean value for GEARS score
            if omit == 1 % PCA features removed
                file = strcat('randForest',num2str(nTrees),'TreesRoundCVPartNoPCA',time,'.mat');
            elseif omit == 2 % force features removed
                file = strcat('randForest',num2str(nTrees),'TreesRoundCVPartNoForce',time,'.mat');
            elseif omit == 3 % PCA and force features removed
                file = strcat('randForest',num2str(nTrees),'TreesRoundCVPartNoForcePCA',time,'.mat');
            end
        end
    elseif TrainTest == 1 % use subject familiarity for train/test split 
        if rounding == 0 % use median value for GEARS scores
            if omit == 1 % PCA features removed
                file = strcat('randForest',num2str(nTrees),'TreesMedSubPartNoPCA',time,'.mat');
            elseif omit == 2 % force features removed
                file = strcat('randForest',num2str(nTrees),'TreesMedSubPartNoForce',time,'.mat');
            elseif omit == 3 % PCA and force features removed
                file = strcat('randForest',num2str(nTrees),'TreesMedSubPartNoForcePCA',time,'.mat');
            end
        elseif rounding == 1 % use mean value for GEARS scores 
            if omit == 1 % PCA features removed
                file = strcat('randForest',num2str(nTrees),'TreesRoundSubPartNoPCA',time,'.mat');
            elseif omit == 2 % force features removed
                file = strcat('randForest',num2str(nTrees),'TreesRoundSubPartNoForce',time,'.mat');
            elseif omit == 3 % PCA and force features removed
                file = strcat('randForest',num2str(nTrees),'TreesRoundSubPartNoForcePCA',time,'.mat');
            end
        end
    end
    ...
    if exist(file,'file')
        ...
        if rounding == 0 % features with median GEARS scores
            if ~exist(strcat('featuresMed',featOF,folder,'.mat'),'file')
                error('Quitting. Please run FowardSelectionOF.m')
            else
                load(strcat('featuresMed',featOF,folder,'.mat'));
            end
        elseif rounding == 1 % features with mean GEARS scores 
            if ~exist(strcat('featuresMean',featOF,folder,'.mat'),'file')
                error('Quitting. Please run FowardSelectionOF.m')
            else
                load(strcat('featuresMean',featOF,folder,'.mat'));
            end
        end       
