...
%%%% This version omits certain features 

function ensembleTestSelectFeaturesRoundCVPartOF(rounding,TrainTest,loocv,time,folder,omit)
    tic
    clearvars -except rounding loocv TrainTest time folder omit;
    addpath('LIBSVM');
    ...
    if rounding == 0 % features with median GEARS scores
        if ~exist(strcat('SelectFeaturesMed',featOF,folder,'.mat'),'file')
            error('Quitting EnsembleTest. Please run FowardSelection.m')
        else
            load(strcat('featuresMed',featOF,folder,'.mat'));
            load(strcat('SelectFeaturesMed',featOF,folder,'.mat'));
        end
    elseif rounding == 1 % features with mean GEARS scores
        if ~exist(strcat('SelectFeaturesMean',featOF,folder,'.mat'),'file')
            error('Quitting EnsembleTest. Please run FowardSelection.m')
        else
            load(strcat('featuresMean',featOF,folder,'.mat'));
            load(strcat('SelectFeaturesMean',featOF,folder,'.mat'));
        end
    end
%         load features.mat;
...
%     check_err(pred_val(:,5), ratings_val(:,5), IndThresh);
    ...
    if TrainTest == 0
        if omit == 1 % PCA features removed
            filepart = 'CVPartNoPCA';
        elseif omit == 2 % force features removed
            filepart = 'CVPartNoForce';
        elseif omit == 3 % PCA and force features removed
            filepart = 'CVPartNoForcePCA';
        end
    elseif TrainTest == 1
        if omit == 1 % PCA features removed
            filepart = 'SubPartNoPCA';
        elseif omit == 2 % force features removed
            filepart = 'SubPartNoForce';
        elseif omit == 3 % PCA and force features removed
            filepart = 'SubPartNoForcePCA';
        end 
    end
